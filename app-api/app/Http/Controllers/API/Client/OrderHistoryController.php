...
class OrderHistoryController extends Controller
{
   ...
   */
  public function getSalesOrderRecord(Request $request)
  {
    try {
      $user_id = auth()->user()->id;
      $this->response_data = OrderProduct::select([
        'order_products.*',
        ...
        ->where(['order_products.seller_id' => $user_id])
      ...
      $data = [
        'api_name' => 'OrderHistoryController::getSalesOrderList',
        'errors' => $e->getMessage(),
      ];
  ...
  public function getSaleOrderDetails(Request $request)
  {
    try {
      ...
      if ($order) {
        $order_items[] = json_decode($order->product);
        $this->response_data = $order;
        $this->response_data->order_items = $order_items;
        $this->response_code = 200;
        $message = $this->mCode->getMessage('sale_order_detail_fetch_success');
      } else {
        $this->response_code = 200;
        $message = $this->mCode->getMessage('no_sale_order_detail_fount');
      }
    } catch (\Exception $e) {
      $data = [
        'api_name' => 'OrderHistoryController::getSaleOrderDetails',
        'errors' => $e->getMessage(),
      ];
      $this->addApiExceptionInDb($data);
      $message = $this->mCode->getMessage('exception_error');
    ...
    try {
      ...
      if ($validator->fails()) {
        ...
        $order = OrderProduct::select([
          ...
          ->leftJoin('orders', 'orders.id', '=', 'order_products.order_id')
          ->where(['order_products.seller_id' => $user_id])
          ->where(['order_products.id' => $request->row_id])->first();
      ...
      $data = [
        'api_name' => 'OrderHistoryController::updateSaleOrderStatus',
        'errors' => $e->getMessage(),
      ];
      $this->addApiExceptionInDb($data);
      $message = $this->mCode->getMessage('exception_error');
  ...
  public function updateOrderStatus(Request $request)
  {
    try {
      ...
      if ($validator->fails()) {
        ...
        // Order::where(['orders.order_id' => $request->order_id])->update([
        //   'transaction_id' => $request->transaction_id,
        //   'payment_status' => 'success',
        //   'status' => 'completed',
        // ]);
        // $order = Order::where(['orders.order_id' => $request->order_id])->first();
        // if ($order) {
        OrderProduct::where(['order_id' => $request->row_id])->update([
          'status' => $request->order_status,
        ]);
        // }
        $user_id = auth()->user()->id;
   ...
   * @return JSON response 
   */
  public function getPurchaseOrderRecord_group_order(Request $request)
  {
    try {
      $user_id = auth()->user()->id;
      $data_rows = Order::where(['orders.user_id' => $user_id])
      ...
      $data = [
        'api_name' => 'OrderHistoryController::getPurchaseOrderRecord',
        'errors' => $e->getMessage(),
      ];
      $this->addApiExceptionInDb($data);
      $message = $this->mCode->getMessage('exception_error');
   ...
   * @return JSON response 
   */
  public function getPurchaseOrderRecord(Request $request)
  {
    try {
      $user_id = auth()->user()->id;
